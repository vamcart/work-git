...
class categories {
	...
	function insert_category($categories_data, $dest_category_id, $action = 'insert') {
		...
		$check_categories_url_query = vam_db_query("SELECT categories_url FROM ".TABLE_CATEGORIES." WHERE categories_url= '".make_alias($categories_data['categories_name'][$_SESSION['languages_id']])."'");
		if (vam_db_num_rows($check_categories_url_query)) {

		$alias = make_alias($categories_data['categories_name'][$_SESSION['languages_id']].'-'.vam_RandomString(3));
        }
		...
		if ($categories_image = vam_try_upload('categories_image', DIR_FS_CATALOG_IMAGES.'categories/')) {
			...
			$categories_image_name = strtolower(vam_RandomString(16).'.'.$cnsuffix);
...
// End Products Specifications
	...
	function insert_product($products_data, $dest_category_id, $action = 'insert') {
		...
		$check_product_url_query = vam_db_query("SELECT products_page_url FROM ".TABLE_PRODUCTS." WHERE products_page_url= '".make_alias($products_data['products_name'][$_SESSION['languages_id']])."'");
		if (vam_db_num_rows($check_product_url_query)) {

		$alias = make_alias($products_data['products_name'][$_SESSION['languages_id']].'-'.vam_RandomString(3));
        }
...
/*		if (!$products_id || $products_id == '') {
			$new_pid_query = vam_db_query("select max(products_id) as max_id from '".TABLE_PRODUCTS."'");
			$new_pid_query_values = vam_db_fetch_array($new_pid_query);
			$products_id = $new_pid_query_values['max_id'];
		}
            */    
                // Products URL begin
		$sql_data_array = array (
		...
		'customers_balance_added_products' => vam_db_prepare_input($products_data['customers_balance_added_products']),
...
// EOF Image subdirectories
			...
			$products_image_name = strtolower(vam_RandomString(16).'_0.'.$nsuffix);
// BOF Image subdirectories
...
// EOF Image subdirectories
				...
				$products_image_name = strtolower(vam_RandomString(16).'_'. ($img +1).'.'.$nsuffix);
// BOF Image subdirectories
