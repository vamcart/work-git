...
$default_these[] = 'v_shipping_status_name';
...
if ($row['v_products_id']) {
        ...
        // if the filelayout says we need a manufacturers name, get it
        if (isset($filelayout['v_shipping_status_name'])){
            if ($row['v_shipping_status_id'] != ''){
                $sql2 = "SELECT shipping_status_name
                    FROM ".TABLE_SHIPPING_STATUS."
                    WHERE
                    shipping_status_id = " . $row['v_shipping_status_id']
                    ;
                $result2 = vam_db_query($sql2);
                $row2 =  vam_db_fetch_array($result2);
                $row['v_shipping_status_name'] = $row2['shipping_status_name'];
            }
        }
      ...
      foreach ($attribute_options_array as $attribute_options) {
                ...
                for ($i=0, $n=sizeof((array)$languages); $i<$n; $i++) {
                    $lid = $languages[$i]['id'];
...
//			echo '<pre>';var_dump($mo_image);echo '</pre>';
			for($i=0, $n=sizeof((array)$mo_image); $i<$n; $i++) {
...
if (!empty($_POST['localfile']) or (isset($_FILES['usrfl']) && isset($_GET['split']) && $_GET['split']==0)) {
    ...
    if (EP_EXCEL_SAFE_OUTPUT == true) {
        ...
        if (EP_EXCEL_SAFE_OUTPUT_ALT_PARCE == false) {
            ...
            {
                unset($line[(sizeof((array)$line)-1)]);  // remove EOREOR at the end of the array
        ...
        $newreaded .= $read;
            ...
            unset($readed[sizeof((array)$readed)-1]);    
            // parse it all!
...
<div id="export">			
        ...
        <p><!-- Download file links -  Add your custom fields here -->
          ...
          $cells[0][] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . vam_draw_checkbox_field('epcust_shippingtime', 'show', (!empty($_GET['epcust_shippingtime'])?true:false)) . '</td><td class="smallText"> '.TEXT_EASYPOPULATE_LABEL_SHIPPINGTIME.'</td></tr></table>');
...
function ep_create_filelayout($dltype, $attribute_options_array, $languages, $custom_fields){
    ...
    case 'full':
        ...
        $filelayout['v_shipping_status_name'] = $iii++;
        ...
        $filelayout_sql = "SELECT
            ...
            p.products_shippingtime as v_shipping_status_id,
...
// EOF mo_image
    ...
    case 'custom':
        ...
        if (!empty($_GET['epcust_shippingtime'])) { $filelayout['v_shipping_status_name'] = $iii++; }
...
//        $v_products_id    =    $items[$filelayout['v_products_id']];
    ...
    // //////////////////////////////////
        ...
        if ((sizeof((array)$row) > 0) && ($_POST['imput_mode'] == "normal" || $_POST['imput_mode'] == "update")) {
            $process_product = true;
		// For Delete Only option (product exists) & (v_status = EP_DELETE_IT) & (Delete Only)
		} elseif ((sizeof((array)$row) > 0) && ($items[$filelayout['v_status']] == EP_DELETE_IT) && ($_POST['imput_mode'] == "delete")) {
            $process_product = true;			
        } elseif ((sizeof((array)$row) == 0) && ($_POST['imput_mode'] == "normal" || $_POST['imput_mode'] == "addnew")) {
            $process_product = true;
        } else {
        ...
        if ($process_product == true) {
            ...
            // OK, we need to convert the manufacturer's name into id's for the database
            if ( isset($v_shipping_status_name) && $v_shipping_status_name != '' ){
                $sql = "SELECT shs.shipping_status_id
                    FROM ".TABLE_SHIPPING_STATUS." as shs
                    WHERE shs.shipping_status_name = '" . vam_db_input($v_shipping_status_name) . "'";
                $result = vam_db_query($sql);
                $row =  vam_db_fetch_array($result);
                if ( $row != '' ){
                    foreach( $row as $item ){
                        $v_shipping_status_id = $item;
                    }
                } else {
                    // to add, we need to put stuff in categories and categories_description
                    $sql = "SELECT MAX( shipping_status_id) max FROM ".TABLE_SHIPPING_STATUS;
                    $result = vam_db_query($sql);
                    $row =  vam_db_fetch_array($result);
                    $max_shps_id = $row['max']+1;
                    // default the id if there are no manufacturers yet
                    if (!is_numeric($max_shps_id) ){
                        $max_shps_id=1;
                    }
                    ...
                    */
                    ...
                    // Comment this query out if you have an older 2.2 codebase
                    $sql = "INSERT INTO ".TABLE_SHIPPING_STATUS."(
                        shipping_status_id,
                        language_id,
                        ...
                        $max_shps_id,
                        '1',
                        '".vam_db_input($v_shipping_status_name)."',
                        ''
                        )";
                    $result = vam_db_query($sql);
                    $v_shipping_status_id = $max_shps_id;
                  ...
                  while ($row = vam_db_fetch_array($result)) {
                    ...
                    $query = "UPDATE ".TABLE_PRODUCTS."
                              SET
                                ...
                                products_shippingtime = ".(!empty($v_shipping_status_id)?$v_shipping_status_id:'NULL').", 
