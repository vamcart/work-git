...
if ($categories['categories_image'] != '')
  ...
  $default->assign('navtrail',$breadcrumb->trail(' &raquo; '));
...
elseif ($category_depth == 'products' || $_GET['manufacturers_id']) {
  ...
  if (vam_db_num_rows($filterlist_query, true) >= 1) {
    ...
    $manufacturer_sort .= '<a href="'.vam_href_link(FILENAME_DEFAULT, 'cat='.$current_category_id.'&filter_id='.$filterlist['id']).'"><span class="btn btn-inverse badge badge-pill badge-light rounded-pill">' . $filterlist['name'] . '</span></a> ';
    } else {
    $manufacturer_sort .= '<a href="'.vam_href_link(FILENAME_DEFAULT, 'filter_id='.$filterlist['id'].'&manufacturers_id='.(int)$_GET['manufacturers_id']).'"><span class="btn btn-inverse badge badge-pill badge-light rounded-pill">' . $filterlist['name'] . '</span></a> ';
    }
    }
    $manufacturer_sort .= '<a href="'.vam_href_link(FILENAME_DEFAULT, 'cat='.$current_category_id).'"><span class="btn btn-inverse badge badge-pill badge-light rounded-pill">' . TEXT_ALL_MANUFACTURERS . '</span></a> ';
