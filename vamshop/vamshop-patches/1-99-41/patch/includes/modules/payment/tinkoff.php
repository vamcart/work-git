...
// class constructor
    function __construct() {
      ...
      $this->code = 'tinkoff';
      $this->title = defined('MODULE_PAYMENT_TINKOFF_TEXT_TITLE') ? MODULE_PAYMENT_TINKOFF_TEXT_TITLE : false;
      $this->public_title = defined('MODULE_PAYMENT_TINKOFF_TEXT_PUBLIC_TITLE') ? MODULE_PAYMENT_TINKOFF_TEXT_PUBLIC_TITLE : false;
      $this->description = defined('MODULE_PAYMENT_TINKOFF_TEXT_ADMIN_DESCRIPTION') ? MODULE_PAYMENT_TINKOFF_TEXT_ADMIN_DESCRIPTION : false;
      $this->icon = DIR_WS_ICONS . 'tinkoff.png';
      $this->sort_order = defined('MODULE_PAYMENT_TINKOFF_SORT_ORDER') ? MODULE_PAYMENT_TINKOFF_SORT_ORDER : null;
      $this->enabled = ((defined('MODULE_PAYMENT_TINKOFF_STATUS') && MODULE_PAYMENT_TINKOFF_STATUS == 'True') ? true : false);
...
// Выписываем qiwi счёт для покупателя
        ...
        $checkout_error = vam_href_link(FILENAME_CHECKOUT, 'payment_error=Ошибка проведения платежа. Попробуйте оплатить ещё раз, убедившись, что данные карты указаны верное и на карте есть необходимая сумма для оплаты.', 'SSL');
        $checkout_success = vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL');
        $checkout_notification = vam_href_link('tinkoff.php', '', 'SSL'); 

        $token = hash('sha256', $orderAmount.$orderId.$checkout_error.$checkout_notification.$orderId.MODULE_PAYMENT_TINKOFF_PASSWORD.$checkout_success.MODULE_PAYMENT_TINKOFF_TERMINAL_KEY);
        ...
        $requestParams['NotificationURL'] = $checkout_notification;
        $requestParams['SuccessURL'] = $checkout_success;
        $requestParams['FailURL'] = $checkout_error;
