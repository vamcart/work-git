...
////
// Output a multiple select form pull down menu
  function vam_draw_manufacturers_multi_pull_down_menu ($name, $values, $default = array(), $parameters = '', $required = false) {
    $field = '<select name="' . vam_output_string ($name) . '"';
	...
	while ($products = vam_db_fetch_array($products_query)) {
			$field .= '<option value="' . vam_output_string ($products['manufacturers_id']) . '"';
      if (in_array ($products['manufacturers_id'], (array) $default) ) {
      //if (in_array_column($products['products_id'], "id", $default) ) {
			$field .= ' SELECTED';
      }

			$field .= '>' . vam_output_string ($products['manufacturers_name'], array ('"' => '&quot;', '\'' => '&#039;', '<' => '&lt;', '>' => '&gt;')) . '</option>';
