...
// class constructor
    public function __construct() {
      ...
      $this->code = 'yookassa';
      $this->title = defined('MODULE_PAYMENT_YOOKASSA_TEXT_TITLE') ? MODULE_PAYMENT_YOOKASSA_TEXT_TITLE : false;
      $this->public_title = defined('MODULE_PAYMENT_YOOKASSA_TEXT_PUBLIC_TITLE') ? MODULE_PAYMENT_YOOKASSA_TEXT_PUBLIC_TITLE : false;
      $this->description = defined('MODULE_PAYMENT_YOOKASSA_TEXT_ADMIN_DESCRIPTION') ? MODULE_PAYMENT_YOOKASSA_TEXT_ADMIN_DESCRIPTION : false;
      $this->icon = DIR_WS_ICONS . 'yookassa.png';
      $this->sort_order = defined('MODULE_PAYMENT_YOOKASSA_SORT_ORDER') ? MODULE_PAYMENT_YOOKASSA_SORT_ORDER : null;
      $this->enabled = ((defined('MODULE_PAYMENT_YOOKASSA_STATUS') && MODULE_PAYMENT_YOOKASSA_STATUS == 'True') ? true : false);
...
//echo 'test'.number_format($order->info['total'], 0, '.', '').'<br />';	    
      ...
      $ot_query = vam_db_query("select class, value from " . TABLE_ORDERS_TOTAL . " where orders_id = " . $order_id . "");
      $ot_array = array();
      while ($ot = vam_db_fetch_array($ot_query)) {
        $ot_array[$ot['class']] = $ot['value'];
      }
//      echo '<pre>';var_export($ot_array);echo '</pre>';
      if (!isset($ot_array['ot_shipping'])) $ot_array['ot_shipping'] = 0;
      $subtotal_discounted = ($ot_array['ot_shipping']+$ot_array['ot_subtotal'])-$ot_array['ot_total'];
      $subtotal_discounted = $ot_array['ot_subtotal']-$subtotal_discounted;
//      echo '<pre>';var_export($subtotal_discounted);echo '</pre>';
      $rate = $subtotal_discounted/$ot_array['ot_subtotal'];
//      echo '<pre>';var_export($rate);echo '</pre>';
      $n = sizeof($order->products)-1;
      $total_calc = 0;
...
//        echo '<pre>';var_export($price);echo '</pre>';
        $price = round($price*$rate, 0);
//        echo '<pre>';var_export($price);echo '</pre>';
        if ($i == $n) {
          $price = ($subtotal_discounted-$total_calc)/$order->products[$i]['qty'];
        }
//        echo '<pre>';var_export($price);echo '</pre>';
        $total_calc += $price*$order->products[$i]['qty'];
        $id_tax = 1;

//echo number_format($price, 2, '.', '').'<br />';

                $receipt['items'][] = array(
                    'description' => mb_substr($product['name'], 0, 128,'UTF-8'),
                    //'description' => rawurlencode(substr($product['name'], 0, 128)),
...
//echo number_format($order->info['shipping_cost'], 2, '.', '').'<br />';

            if ($order->info && $order->info['shipping_cost'] > 0) {
                $id_tax = 1;
                $receipt['items'][] = array(
                    //'description' => rawurlencode(substr('Доставка - ' . $order->info['shipping_method'], 0, 128)),
                    'description' => mb_substr('Доставка - ' . $order->info['shipping_method'], 0, 128,'UTF-8'),
                    'quantity' => 1,
                    'amount' => array(
