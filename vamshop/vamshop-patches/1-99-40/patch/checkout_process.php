...
for ($i = 0, $n = sizeof($order_totals); $i < $n; $i ++) {
	...
	//kovalsky
	if ($order_totals[$i]['code'] == 'ot_balance'){
		$minus_balance = $order_totals[$i]['value'];
		...
		$res_ballance = $old_balance - $minus_balance;
		...
		('".(int)$_SESSION['customer_id']."','".(int)$old_balance."','".(int)$res_ballance ."',now(),'Оплата заказа № ".$insert_id."','Списание ".$minus_balance."') ");
						
		vam_db_query("update " . TABLE_CUSTOMERS . " set customers_balance = " . (int)$res_ballance . " where customers_id = '" .(int)$_SESSION['customer_id']."'");
...
for ($i = 0, $n = sizeof($order->products); $i < $n; $i ++) {
	...
	$products_query = vam_db_query("select p.customers_balance_added_products, p.manufacturers_id from " . TABLE_PRODUCTS . " p where p.products_id='" . vam_get_prid($order->products[$i]['id']) . "'");
	$products = vam_db_fetch_array($products_query);
	$products_balance_added = $products['customers_balance_added_products'];
	
	if ($products['manufacturers_id'] !=''){
		
		$manufacturer_query = vam_db_query("select customers_balance_added_manufacturers from " . TABLE_MANUFACTURERS . " where manufacturers_id = '" . $products['manufacturers_id'] . "'");
		$manufacturer_balance = vam_db_fetch_array($manufacturer_query);
		$manufacturer_balance_added = $manufacturer_balance['customers_balance_added_manufacturers'];
	
	}
	
	$balance_addad = KOVALSKY_BONUS_PERSENT;
	if($manufacturer_balance_added > 0.001){
		$balance_addad = $manufacturer_balance_added;
	}
	
	if($products_balance_added > 0.001){
		$balance_addad = $products_balance_added;
	}
	
	$products_balance_added = '';	
	$manufacturer_balance_added = '';


	$sql_data_array = array ('orders_id' => $insert_id, 
                            'products_id' => vam_get_prid($order->products[$i]['id']),
                            'customers_balance_added' =>  $balance_addad, 
